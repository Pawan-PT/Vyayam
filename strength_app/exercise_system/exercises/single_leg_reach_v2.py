"""
SingleLegReach V2 - Standing single-leg reach in multiple directions — Star Excursion lite
...
class SingleLegReachV2:
    """SingleLegReach — Standing single-leg reach in multiple directions — Star Excursion lite

    Level: L3 advanced
    Category: balance
    Movement Pattern: single-leg-stability
    Target: Hip stabilizers, ankle complex, hip abductors and adductors

    Biomechanics:
    - Stand on one leg; reach free foot in anterior, posteromedial, posterolateral directions
    - Reach as far as possible while maintaining stance leg alignment
    - Track reach distance and quality for each direction
    ...
    def calculate_angles(self, analyzer, results, shape):
        ...
        reach_distance = abs(ra[0] - la[0]) / shape[1] if ra and la else 0
        hip_drop = lh[1] - rh[1] if lh and rh else 0
    ...
    def get_target_poses(self):
        ...
        return {
            'standing': {'stance_knee': 165, 'tolerance': 15},
            'reaching': {'reach_distance': 0.3, 'tolerance': 0.1},
            'returning': {'stance_knee': 165, 'tolerance': 15},
        }

    def validate_form(self, angles, phase):
        """Check form against exercise-specific standards."""
        feedback = {}
        hip_d = angles.get('hip_drop', 0)
        if abs(hip_d) > 25:
            feedback['hip_drop'] = {'status': 'error', 'message': 'Hip dropping — maintain pelvis level'}
        stance = angles.get('stance_knee', 165)
        if stance < 140:
            feedback['stance_bend'] = {'status': 'warning', 'message': 'Slight knee bend okay — do not collapse'}
        return feedback
    ...
    def update_rep_counter(self, angles, feedback, voice):
        ...
        if self.phase == "standing" and reach > 0.2:
            self.phase = "reaching"
            self.tempo_detector.start_phase('reaching')
        elif self.phase == "reaching" and reach > 0.3:
            self.phase = "returning"
        elif self.phase == "returning" and reach < 0.1:
            rep_done = True
            self.phase = "standing"
