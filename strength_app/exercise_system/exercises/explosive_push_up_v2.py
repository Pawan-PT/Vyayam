"""
ExplosivePushUp V2 - Push-up with hands leaving floor — upper body plyometric power
...
class ExplosivePushUpV2:
    """ExplosivePushUp — Push-up with hands leaving floor — upper body plyometric power

    Level: L4 elite
    Category: power
    Movement Pattern: push-power
    Target: Pectoralis major, triceps, deltoid, serratus anterior

    Biomechanics:
    - Descend with control, then explode to push hands off floor
    - Land with elbows soft to absorb and immediately descend for next rep
    - Body maintains a plank line throughout — no hip sag or pike
    ...
    def calculate_angles(self, analyzer, results, shape):
        ...
        shoulder_elevation = (shape[0] - ls[1]) / shape[0] if ls else 0.5
        return {
            'left_knee': left_knee,
    ...
    def get_target_poses(self):
        ...
        return {
            'top': {'elbow_angle': 175, 'tolerance': 10},
            'lowering': {'elbow_angle': 130, 'tolerance': 20},
            'bottom': {'elbow_angle': 90, 'tolerance': 15},
            'exploding': {'elbow_angle': 160, 'tolerance': 15},
        }
    ...
    def validate_form(self, angles, phase):
        ...
        if body < 158:
            feedback['body_line'] = {'status': 'error', 'message': 'Maintain plank line — no hip sag'}
        elbow = angles.get('elbow_angle', 90)
        if elbow > 120 and phase == 'bottom':
            feedback['depth'] = {'status': 'warning', 'message': 'Lower chest closer to floor before exploding'}
        return feedback
    ...
    def update_rep_counter(self, angles, feedback, voice):
        ...
        rep_done = False
        warnings = []
        elbow = angles.get('elbow_angle', 175)
        shoulder_el = angles.get('shoulder_elevation', 0.5)

        if self.phase == "top" and elbow < 150:
            self.phase = "lowering"
            self.tempo_detector.start_phase('lowering')
        elif self.phase == "lowering" and elbow < 100:
            self.phase = "bottom"
        elif self.phase == "bottom" and elbow > 140:
            self.phase = "exploding"
            self.tempo_detector.start_phase('exploding')
        elif self.phase == "exploding" and elbow > 168:
            rep_done = True
            self.phase = "top"
