"""
Wall Handstand Push-Up - Full handstand push-up against wall
...
class WallHandstandPushUpV2:
    """Wall Handstand Push-Up - Level: advanced | Push pattern (peak push skill)

    Biomechanics:
    - Full inversion against wall; head descends to floor at bottom
    - Shoulder angle tracks near 180° at top, ~90° at bottom
    - Body alignment must stay straight — avoid lumbar arch
    - Controlled eccentric descent is critical for joint safety
    ...
    def calculate_angles(self, analyzer, results, shape):
        """Shoulder angle through full ROM + elbow angle for depth."""
    ...
    def get_target_poses(self):
        return {
            ...
            'down':   {'avg_elbow': 90,  'body_align': 175, 'tolerance': 15},
            # DA-EX-phases: state machine reaches 'bottom'; it was missing
            # from the scored targets (KeyError on every rep at depth).
            'bottom': {'avg_elbow': 90,  'body_align': 175, 'tolerance': 15},
            'up':     {'avg_elbow': 170, 'body_align': 175, 'tolerance': 10},
        }
    ...
    def validate_form(self, angles, phase):
        ...
        if ba < 165:
            feedback['alignment'] = JointFeedback('hip', FormStatus.WARNING,
                'Keep body straight — no banana arch')
        if phase == 'down' and ea < 80:
            feedback['depth'] = JointFeedback('elbow', FormStatus.GOOD,
                'Head near floor — full ROM achieved')
        return feedback
    ...
    def update_rep_counter(self, angles, feedback, voice):
        ...
        elif self.phase == 'down' and avg_elbow < 95:
            self.phase = 'bottom'
        elif self.phase == 'bottom' and avg_elbow > 105:
            self.phase = 'up'
            self.tempo_detector.start_phase('up')
        elif self.phase == 'up' and avg_elbow >= 162:
            rep_done = True
            self.phase = 'start'
