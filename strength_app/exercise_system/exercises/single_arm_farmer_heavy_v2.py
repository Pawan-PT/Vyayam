"""
SingleArmFarmerHeavy V2 - Heavy unilateral carry — maximal lateral core demand
...
class SingleArmFarmerHeavyV2:
    """SingleArmFarmerHeavy — Heavy unilateral carry — maximal lateral core demand

    Level: L3 advanced
    Category: carry
    Movement Pattern: anti-lateral-flexion
    Target: Quadratus lumborum, obliques, gluteus medius, trapezius, forearms

    Biomechanics:
    - Heavier load than standard suitcase carry — maximal lateral stability challenge
    - Zero lateral trunk deviation toward or away from weight
    - Gait must remain smooth — no waddling, no limp, no trunk oscillation
    ...
    def calculate_angles(self, analyzer, results, shape):
        ...
        shoulder_drop = ls[1] - rs[1] if ls and rs else 0
    ...
    def validate_form(self, angles, phase):
        ...
        if lean < 160:
            feedback['lateral_lean'] = {'status': 'error', 'message': 'Reduce weight or brace harder — excessive lean'}
        hip_d = angles.get('hip_drop', 0)
        if abs(hip_d) > 20:
            feedback['hip_drop'] = {'status': 'warning', 'message': 'Keep hips level — gluteus medius is failing'}
        return feedback
