"""
Towel Row - Loop towel around sturdy object, sit back and row
...
class TowelRowV2:
    """Towel Row - Level: foundation | Pull pattern (no equipment)

    Biomechanics:
    - Towel looped around table leg/pole; sit back and pull chest to object
    - Body held straight — no hip sagging during pull
    - Elbow angle to ~90° at top position
    - Scapular retraction key at peak contraction
    ...
    def calculate_angles(self, analyzer, results, shape):
        """Elbow angle + trunk position for towel row."""
    ...
    def get_target_poses(self):
        return {
            'start': {'avg_elbow': 170, 'trunk_angle': 50, 'tolerance': 15},
            'pull':  {'avg_elbow': 90,  'trunk_angle': 50, 'tolerance': 15},
            # DA-EX-phases: 'top' and 'lower' were reachable by the state
            # machine but missing from targets (KeyError mid-rep).
            'top': {'avg_elbow': 90,  'trunk_angle': 50, 'tolerance': 15},
    ...
    def validate_form(self, angles, phase):
        ...
        if ta > 75:
            feedback['lean'] = JointFeedback('hip', FormStatus.WARNING,
                'Lean back more — body at 45° to floor')
        if phase == 'top':
            feedback['squeeze'] = JointFeedback('shoulder', FormStatus.GOOD,
