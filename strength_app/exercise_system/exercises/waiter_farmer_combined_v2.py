"""
WaiterFarmerCombined V2 - Simultaneous overhead and side carry — full-body alignment challenge
...
class WaiterFarmerCombinedV2:
    """WaiterFarmerCombined — Simultaneous overhead and side carry — full-body alignment challenge

    Level: L3 advanced
    Category: carry
    Movement Pattern: overhead-stability
    Target: Rotator cuff, obliques, trapezius, core, gluteus medius

    Biomechanics:
    - One arm overhead (waiter), other at side (farmer) — both carrying weight
    - Trunk must remain perfectly upright with zero lateral lean
    - The overhead arm challenges the rotator cuff; the side arm challenges the QL
    ...
    def calculate_angles(self, analyzer, results, shape):
        ...
        overhead_lock = analyzer.calculate_angle(ls, le, lw)
        trunk_lean = analyzer.calculate_angle(ls, lh, la)
        shoulder_level = abs(ls[1] - rs[1]) if ls and rs else 0
        return {
            'left_knee': left_knee,
            'right_knee': right_knee,
            'avg_knee': avg_knee,
            'overhead_lock': overhead_lock,
    ...
    def validate_form(self, angles, phase):
        ...
        feedback = {}
        elbow = angles.get('overhead_lock', 170)
        if elbow < 155:
            feedback['overhead_elbow'] = {'status': 'error', 'message': 'Lock overhead arm fully'}
        trunk = angles.get('trunk_lean', 175)
        if trunk < 162:
            feedback['trunk'] = {'status': 'warning', 'message': 'Stand tall — no lean in any direction'}
        sh = angles.get('shoulder_level', 0)
        if sh > 25:
            feedback['shoulder_level'] = {'status': 'warning', 'message': 'Keep both shoulders level'}
        return feedback
