"""
Negative Pull-Up - Jump to top, 5-second controlled descent to dead hang
...
class NegativePullUpV2:
    """Negative Pull-Up - Level: building | Pull pattern (bar track, eccentric)

    Biomechanics:
    - Jump or step to chin-over-bar position; lower over 5 seconds
    - Full eccentric through elbow flexion range (~60° to 175°)
    - No kipping — purely eccentric loading
    - Best building exercise for first full pull-up
        ...
        if phase == 'descend':
            ...
            if elapsed < 3:
                feedback['tempo'] = JointFeedback('elbow', FormStatus.WARNING,
                    'Slow down — 5-second descent target')
            else:
                feedback['tempo'] = JointFeedback('elbow', FormStatus.GOOD,
                    f'Good tempo — {elapsed:.1f}s so far')
        return feedback
