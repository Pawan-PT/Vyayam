"""
Hip Hinge Wall V2 - Foundational hinge pattern using wall as glute target
...
class HipHingeWallV2:
    """Hip Hinge Wall - Stand facing away from wall, hinge back until glutes contact wall.
    ...
    Biomechanics:
    - Push hips backward (not knees forward) toward wall
    - Neutral spine throughout — no rounding or hyperextension
    - Soft knee bend (~20°) — knees are not the primary mover
    - Hip angle (trunk-to-thigh) decreases as patient hinges
    ...
    def calculate_angles(self, analyzer, results, shape):
        ...
        # Hip angle: trunk-to-thigh (shoulder→hip→knee)
    ...
    def get_target_poses(self):
        """Hip angle drops from ~180° (erect) to ~100° at the hinge."""
        return {
            'standing':  {'avg_hip': 175, 'avg_knee': 170, 'tolerance': 10},
            'hinging':   {'avg_hip': 135, 'avg_knee': 160, 'tolerance': 15},
            'touch':     {'avg_hip': 100, 'avg_knee': 155, 'tolerance': 15},
            'returning': {'avg_hip': 135, 'avg_knee': 160, 'tolerance': 15},
        }

    def validate_form(self, angles, phase):
        feedback = {}
        knee = angles.get('avg_knee', 170)
        hip = angles.get('avg_hip', 175)

        if phase in ('hinging', 'touch'):
            # Knees should remain relatively straight (soft bend only)
            if knee < 140:
                feedback['knee'] = JointFeedback(
                    joint='knee', status=FormStatus.WARNING,
                    message="Keep knees soft — this is a hip hinge, not a squat"
                )
            if hip > 150 and phase == 'touch':
                feedback['hip'] = JointFeedback(
                    joint='hip', status=FormStatus.WARNING,
                    message="Push hips further back toward wall"
                )
        return feedback
    ...
    def update_rep_counter(self, angles, feedback, voice):
        rep_done = False
        warnings = []
        angles_dict = angles if isinstance(angles, dict) else {'avg_knee': angles, 'avg_hip': 175}
        avg_hip = angles_dict.get('avg_hip', 175)

        if self.phase == "standing" and avg_hip < 155:
            self.phase = "hinging"
            self.tempo_detector.start_phase('hinging')
        elif self.phase == "hinging" and avg_hip < 110:
            self.phase = "touch"
        elif self.phase == "touch" and avg_hip > 120:
            self.phase = "returning"
            self.tempo_detector.start_phase('returning')
        elif self.phase == "returning" and avg_hip >= 165:
            rep_done = True
            self.phase = "standing"
