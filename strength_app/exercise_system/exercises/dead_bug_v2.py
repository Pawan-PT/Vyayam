"""
DeadBug V2 - Supine opposite arm-leg extension for deep core stability
...
class DeadBugV2:
    """DeadBug — Supine opposite arm-leg extension for deep core stability

    Level: L1 foundation
    Category: core
    Movement Pattern: anti-extension
    Target: Transverse abdominis, multifidus, diaphragm

    Biomechanics:
    - Maintain lumbar spine contact with floor throughout — no arch
    - Move opposite arm and leg simultaneously in a slow, controlled arc
    - Exhale fully during the extension to activate TVA
    ...
    def calculate_angles(self, analyzer, results, shape):
        ...
        left_hip_flex = analyzer.smooth_angle(analyzer.calculate_angle(ls, lh, lk), 'left_hip')
        right_hip_flex = analyzer.smooth_angle(analyzer.calculate_angle(rs, rh, rk), 'right_hip')
        trunk_angle = analyzer.calculate_angle(ls, lh, la)
        return {
            'left_knee': left_knee,
    ...
    def get_target_poses(self):
        ...
        return {
            'start': {'avg_knee': 90, 'tolerance': 15},
            'extending': {'left_hip_flex': 160, 'right_hip_flex': 160, 'tolerance': 20},
            'return': {'avg_knee': 90, 'tolerance': 15},
        }

    def validate_form(self, angles, phase):
        """Check form against exercise-specific standards."""
        feedback = {}
        trunk = angles.get('trunk_angle', 180)
        if trunk < 155:
            feedback['lumbar'] = {'status': 'error', 'message': 'Press lower back flat to floor'}
        lhf = angles.get('left_hip_flex', 90)
        rhf = angles.get('right_hip_flex', 90)
        if abs(lhf - rhf) > 30 and phase == 'extending':
            feedback['coordination'] = {'status': 'warning', 'message': 'Extend arm and leg together'}
        return feedback
    ...
    def update_rep_counter(self, angles, feedback, voice):
        ...
        if self.phase == "start" and avg_ext > 130:
            self.phase = "extending"
            self.tempo_detector.start_phase('extending')
        elif self.phase == "extending" and avg_ext > 155:
            self.phase = "return"
        elif self.phase == "return" and avg_ext < 100:
            rep_done = True
            self.phase = "start"
