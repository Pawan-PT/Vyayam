"""
Bodyweight RDL V2 - Romanian deadlift without weight, foundational hamstring hinge
...
class BodyweightRdlV2:
    """Bodyweight RDL - Hip hinge with hands sliding down thighs to teach hamstring loading.
    ...
    Biomechanics:
    - Neutral spine throughout — no rounding at lumbar
    - Hips push back as torso inclines forward
    - Slight knee bend (~15°) — knees remain over ankle
    - Feel hamstring stretch at bottom; drive hips forward to return
        ...
        if phase in ('hinging', 'bottom'):
            if knee < 140:
                feedback['knee'] = JointFeedback(
                    ...
                    message="Too much knee bend — keep knees soft, not squatting"
                )
            if hip > 120 and phase == 'bottom':
                feedback['depth'] = JointFeedback(
                    joint='hip', status=FormStatus.WARNING,
                    message="Hinge deeper — push hips further back"
                )
        return feedback
    ...
    def update_rep_counter(self, angles, feedback, voice):
        rep_done = False
        warnings = []
        angles_dict = angles if isinstance(angles, dict) else {'avg_hip': 175, 'avg_knee': 170}
