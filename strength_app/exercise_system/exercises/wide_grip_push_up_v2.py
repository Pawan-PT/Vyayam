"""
Wide-Grip Push-Up - Hands wider than shoulders for greater chest engagement
...
class WideGripPushUpV2:
    """Wide-Grip Push-Up - Level: building | Push pattern

    Biomechanics:
    - Hands 1.3–1.5x shoulder width; increased chest (pec major) demand
    - Elbows flare slightly more than standard push-up
    - Chest contacts floor — same depth target
    - Hip neutrality maintained throughout
    ...
    def calculate_angles(self, analyzer, results, shape):
        """Elbow angle + wrist spread relative to shoulder width."""
        ...
        # Normalised wrist spread vs shoulder spread
        wrist_spread = abs(lw[0] - rw[0]) if lw and rw else 0
        shldr_spread = abs(ls[0] - rs[0]) if ls and rs else 1
        width_ratio  = round(wrist_spread / (shldr_spread + 1e-6), 2)

        return {
            'left_elbow': left_elbow, 'right_elbow': right_elbow,
            'avg_elbow': avg_elbow, 'hip_angle': hip_angle, 'width_ratio': width_ratio,
    ...
    def get_target_poses(self):
        return {
            'start':  {'avg_elbow': 170, 'width_ratio': 1.4, 'tolerance': 10},
            'down':   {'avg_elbow': 90,  'width_ratio': 1.4, 'tolerance': 15},
            # DA-EX-phases: state machine reaches 'bottom'; it was missing
            # from the scored targets (KeyError on every rep at depth).
            'bottom': {'avg_elbow': 90,  'width_ratio': 1.4, 'tolerance': 15},
            'up':     {'avg_elbow': 170, 'width_ratio': 1.4, 'tolerance': 10},
        }

    def validate_form(self, angles, phase):
        feedback = {}
        wr = angles.get('width_ratio', 1.0)
        ha = angles.get('hip_angle', 180)
        if wr < 1.3:
            feedback['width'] = JointFeedback('shoulder', FormStatus.WARNING,
                'Widen hands — beyond shoulder width')
        if ha < 160:
            feedback['hip'] = JointFeedback('hip', FormStatus.WARNING,
