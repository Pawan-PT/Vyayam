"""
Ring Push Up V2 - Push-up on gymnastic rings for added instability
...
class RingPushUpV2:
    """Ring Push-Up - Push-up on gymnastic rings; unstable surface demands extra stability.

    Level: advanced (L5)
    Category: strength
    Movement Pattern: push
    Target: pectorals, anterior deltoid, triceps, stabilising rotator cuff

    Biomechanics:
    - Rings at approximately push-up height (hands ~6 inches off floor)
    - Greater pectoral activation due to ring rotation at lockout
    - Rings should turn inward (palms face each other) at top
    - Body must remain rigid to prevent ring oscillation
    ...
    def calculate_angles(self, analyzer, results, shape):
        ...
        # Hip sag check: shoulder-hip-knee alignment
    ...
    def validate_form(self, angles, phase):
        ...
        if body < 155:
            feedback['hip_sag'] = JointFeedback(
                joint='hip', status=FormStatus.WARNING,
                message="Keep body rigid — hips sagging"
            )
        return feedback
