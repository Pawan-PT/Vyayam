"""
Renegade Row - Push-up position, alternating dumbbell rows
...
class RenegadeRowV2:
    ...
    Target: latissimus dorsi, rhomboids, anti-rotation core

    Anti-rotation during row is the primary training stimulus; keep hips level.
    """
    ...
    def validate_form(self, angles, phase):
        feedback = {}
        hip_rot = angles.get('hip_rot', 0) if 'hip_rot' in angles else 0
        if hip_rot > 20:
            feedback['rotation'] = JointFeedback(
                joint='hip', status=FormStatus.WARNING,
                message="Resist hip rotation during the row"
            )
        return feedback
