"""
Weighted Pull-Up - Standard pull-up with additional load (backpack/belt)
...
class WeightedPullUpV2:
    """Weighted Pull-Up - Level: advanced | Pull pattern (bar track, loaded)

    Biomechanics:
    - Standard pronated grip pull-up with external load (backpack/dip belt)
    - Strict form required — kipping not permitted with added weight
    - hip_swing monitoring detects momentum-based cheating
    - Full ROM: dead hang → chin over bar → controlled descent
    ...
    - NOSE (0) for chin-clearance detection
    """
    ...
    def calculate_angles(self, analyzer, results, shape):
        """Same as full pull-up; hip alignment checks for no kipping."""
    ...
    def validate_form(self, angles, phase):
        feedback = {}
        hs  = angles.get('hip_swing', 175)
        cc  = angles.get('chin_clear', 0)
        ea  = angles.get('avg_elbow', 180)
        if hs < 155:
            feedback['kip'] = JointFeedback('hip', FormStatus.WARNING,
                'No kipping — hips must stay still')
        if phase == 'hang' and ea < 165:
            feedback['hang'] = JointFeedback('elbow', FormStatus.WARNING,
                'Full dead hang — complete arm extension')
        if phase == 'top' and cc == 0:
            feedback['chin'] = JointFeedback('shoulder', FormStatus.WARNING,
                'Chin over bar — full ROM required')
        return feedback
    ...
    def update_rep_counter(self, angles, feedback, voice):
        ...
        chin_clear = angles.get('chin_clear', 0) if isinstance(angles, dict) else 0

        if self.phase == 'hang' and avg_elbow < 140:
            self.phase = 'pull'
