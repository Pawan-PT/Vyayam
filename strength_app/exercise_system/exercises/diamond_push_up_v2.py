"""
Diamond Push Up V2 - Close-grip push-up with hands forming diamond shape
...
class DiamondPushUpV2:
    """Diamond Push-Up - Hands close together under chest forming a diamond.

    Level: intermediate (L3)
    Category: strength
    Movement Pattern: push
    Target: triceps brachii (primary), anterior deltoid, chest

    Biomechanics:
    - Index fingers and thumbs touch forming a diamond shape on the floor
    - Elbows travel close to torso (not flared) on descent
    - Body in straight plank line from head to heel
    - Full extension at top without locking elbows aggressively
    ...
    def calculate_angles(self, analyzer, results, shape):
        ...
        # Body alignment: shoulder-hip angle
        body_align = analyzer.calculate_angle(ls, lh,
                                              analyzer.get_coords(results,
                                              analyzer.mp_pose.PoseLandmark.LEFT_KNEE, shape))
    ...
    def validate_form(self, angles, phase):
        ...
        if phase == 'down' and elbow > 110:
            feedback['depth'] = JointFeedback(
                ...
                message="Go lower — chest toward hands"
            )
        return feedback
