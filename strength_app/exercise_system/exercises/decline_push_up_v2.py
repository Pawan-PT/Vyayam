"""
Decline Push-Up - Feet elevated, greater anterior deltoid demand
...
class DeclinePushUpV2:
    """Decline Push-Up - Level: intermediate | Push pattern

    Biomechanics:
    - Feet on elevated surface (chair/bench), body angled downward
    - Increases anterior deltoid and upper pec demand
    - Body must remain straight from ankles to head
    - Shoulder angle target ~70–90° throughout movement
    ...
    def calculate_angles(self, analyzer, results, shape):
        """Shoulder angle (anterior delt emphasis) + elbow angle."""
    ...
    def get_target_poses(self):
        return {
            'start':  {'avg_elbow': 170, 'shoulder_angle': 75, 'tolerance': 10},
            'down':   {'avg_elbow': 90,  'shoulder_angle': 90, 'tolerance': 15},
            # DA-EX-phases: state machine reaches 'bottom'; it was missing
            # from the scored targets (KeyError on every rep at depth).
            'bottom': {'avg_elbow': 90,  'shoulder_angle': 90, 'tolerance': 15},
            'up':     {'avg_elbow': 170, 'shoulder_angle': 75, 'tolerance': 10},
        }

    def validate_form(self, angles, phase):
        feedback = {}
        sa = angles.get('shoulder_angle', 0)
        if sa < 55:
            feedback['incline'] = JointFeedback('shoulder', FormStatus.WARNING,
                'Elevate feet higher for proper decline angle')
        if phase == 'down':
            feedback['depth'] = JointFeedback('elbow', FormStatus.GOOD,
                f'Shoulder pressing angle: {sa:.0f}°')
        return feedback
