"""
Archer Push-Up - One arm performs push-up, other arm extends out straight
...
class ArcherPushUpV2:
    """Archer Push-Up - Level: intermediate | Push pattern (unilateral progression)

    Biomechanics:
    - Wide hand placement; one arm bends fully, other arm stays extended
    - Working arm elbow target ~90° at bottom
    - Assisting arm remains near straight (150°+) to minimise contribution
    ...
    def calculate_angles(self, analyzer, results, shape):
        """Track both elbow angles; working arm is the more bent one."""
    ...
    def validate_form(self, angles, phase):
        ...
        if aa < 150:
            feedback['assist_straight'] = JointFeedback('elbow', FormStatus.WARNING,
                'Keep assist arm straight — extend fully')
        if phase == 'down' and wa > 105:
    ...
    def update_rep_counter(self, angle, feedback, voice):
        ...
        elif self.phase == 'down' and working < 105:
            self.phase = 'bottom'
        elif self.phase == 'bottom' and working > 115:
            self.phase = 'up'
            self.tempo_detector.start_phase('up')
        elif self.phase == 'up' and working >= 160:
            rep_done = True
            self.phase = 'start'
