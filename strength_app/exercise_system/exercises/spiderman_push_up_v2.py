"""
Spiderman Push Up V2 - Push-up with alternating knee-to-elbow drive
...
class SpidermanPushUpV2:
    """Spiderman Push-Up - Standard push-up with knee drive to same-side elbow at bottom.

    Level: intermediate (L3)
    Category: strength
    Movement Pattern: push
    Target: pectorals, triceps, hip flexors, obliques

    Biomechanics:
    - As body descends, one knee drives laterally up toward the same-side elbow
    - Knee should reach elbow level at the bottom of the push-up
    - Alternate legs each rep
    - Hip should not rotate excessively on the non-driving side
    ...
    def calculate_angles(self, analyzer, results, shape):
        ...
        # Knee-to-elbow proximity (active side)
        active_elbow = le if self.active_side == 'left' else re
        active_knee = lk if self.active_side == 'left' else rk
        knee_elbow_dist = abs(active_knee[0] - active_elbow[0]) + abs(active_knee[1] - active_elbow[1])

        return {
            'avg_elbow': avg_elbow, 'knee_elbow_dist': knee_elbow_dist,
            'joints_coords': {'ls': ls, 'rs': rs, 'le': le, 're': re,
                               'lw': lw, 'rw': rw, 'lh': lh, 'rh': rh, 'lk': lk, 'rk': rk}
        }
    ...
    def validate_form(self, angles, phase):
        feedback = {}
        dist = angles.get('knee_elbow_dist', 100)
        if phase == 'down' and dist > 60:
            feedback['knee_drive'] = JointFeedback(
                joint='knee', status=FormStatus.INFO,
                message="Drive your knee toward your elbow at the bottom"
            )
        return feedback
