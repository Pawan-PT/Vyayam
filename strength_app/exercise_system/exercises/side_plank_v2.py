"""
SidePlank V2 - Lateral isometric plank for oblique and hip abductor strength
...
class SidePlankV2:
    """SidePlank — Lateral isometric plank for oblique and hip abductor strength

    Level: L2 intermediate
    Category: core
    Movement Pattern: anti-lateral-flexion
    Target: Obliques, gluteus medius, hip abductors, shoulder stabilizers

    Biomechanics:
    - Body forms a straight line from head to heel — no hip sag
    - Stack feet or stagger for stability; elbow directly under shoulder
    - Score each side separately — asymmetry indicates imbalance
    ...
    def calculate_angles(self, analyzer, results, shape):
        ...
        hip_height_diff = abs(lh[1] - rh[1]) if lh and rh else 0
        shoulder_hip_align = analyzer.calculate_angle(ls, lh, lk)
        return {
            'left_knee': left_knee,
            ...
            'shoulder_hip_align': shoulder_hip_align,
            'joints_coords': {
                'lh': lh, 'lk': lk, 'la': la,
                'rh': rh, 'rk': rk, 'ra': ra,
                'ls': ls, 'rs': rs
    ...
    def validate_form(self, angles, phase):
        ...
        if body < 155:
            feedback['hip_sag'] = {'status': 'error', 'message': 'Lift hips — body should be straight'}
        if body > 195:
            feedback['hip_pike'] = {'status': 'warning', 'message': 'Lower hips slightly — avoid pike'}
        hip_diff = angles.get('hip_height_diff', 0)
        if hip_diff > 25:
            feedback['hip_drop'] = {'status': 'error', 'message': 'Keep hips stacked — no forward/back lean'}
        return feedback
