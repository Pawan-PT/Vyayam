"""
Push-Up Plus - Serratus anterior activation push-up with scapular protraction at top
...
class PushUpPlusV2:
    ...
    Target: serratus anterior, pectorals, anterior deltoid

    Standard push-up with added scapular protraction (push-plus) at top.
    ...
    def calculate_angles(self, analyzer, results, shape):
        ...
        # Shoulder protraction proxy: wrist spread relative to shoulder spread
        shoulder_width = abs(ls[0] - rs[0])
        wrist_width = abs(lw[0] - rw[0])
        protraction_ratio = wrist_width / max(1, shoulder_width)
    ...
    def get_target_poses(self):
        return {
            ...
            'down': {'avg_elbow': 170,  'tolerance': 15},
        }

    def validate_form(self, angles, phase):
        feedback = {}
        if phase == 'up':
            prot = angles.get('protraction_ratio', 1.0)
            if prot < 1.05:
                feedback['protraction'] = JointFeedback(
                    joint='shoulder', status=FormStatus.INFO,
                    message="Plus phase: push hands apart, round upper back at top"
                )
        return feedback
