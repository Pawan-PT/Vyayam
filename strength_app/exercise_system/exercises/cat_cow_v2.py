"""
CatCow V2 - Quadruped spinal flexion-extension flow — segmental spine mobility
...
class CatCowV2:
    """CatCow — Quadruped spinal flexion-extension flow — segmental spine mobility

    Level: mobility
    Category: mobility
    Movement Pattern: spinal-mobility
    Target: All spinal segments, multifidus, erector spinae, rectus abdominis

    Biomechanics:
    - Cat: exhale, tuck pelvis, round spine from tailbone to neck
    - Cow: inhale, tilt pelvis, extend spine from tailbone to neck
    - Movement should be segmental and fluid — not just hip/neck compensation
    ...
    def calculate_angles(self, analyzer, results, shape):
        ...
        spine_curve = analyzer.calculate_angle(ls, lh, la)
        pelvis_tilt = analyzer.calculate_angle(ls, lh, lk)
        shoulder_position = ls[1] / shape[0] if ls else 0.5
        return {
            'left_knee': left_knee,
    ...
    def get_target_poses(self):
        ...
        return {
            'neutral': {'spine_curve': 175, 'tolerance': 15},
            'cat': {'spine_curve': 145, 'tolerance': 20},
            'cow': {'spine_curve': 200, 'tolerance': 20},
        }

    def validate_form(self, angles, phase):
        """Check form against exercise-specific standards."""
        feedback = {}
        spine = angles.get('spine_curve', 175)
        if 160 < spine < 190 and phase in ('cat', 'cow'):
            feedback['range'] = {'status': 'warning', 'message': 'Exaggerate more — full flexion in cat, full extension in cow'}
        return feedback
    ...
    def update_rep_counter(self, angles, feedback, voice):
        ...
        if self.phase == "neutral" and spine < 160:
            self.phase = "cat"
            self.tempo_detector.start_phase('cat')
        elif self.phase == "cat" and spine > 190:
            self.phase = "cow"
            self.tempo_detector.start_phase('cow')
        elif self.phase == "cow" and spine < 170:
            rep_done = True
            self.phase = "neutral"
