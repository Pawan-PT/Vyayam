"""
Good Morning V2 - Hip hinge with hands behind head
...
class GoodMorningV2:
    """Good Morning - Hip hinge with hands clasped behind head for spinal control.

    Level: building
    Category: strength
    Movement Pattern: hinge
    Target: hamstrings, glutes, spinal erectors, core

    Biomechanics:
    - Hands behind head (not pulling on neck) adds difficulty to spine neutral position
    - Hinge from hips, not waist — lumbar spine must not flex
    - Controlled tempo — slow down, pause briefly, drive up with glutes
    ...
    def get_target_poses(self):
        return {
            'standing':  {'avg_hip': 175, 'tolerance': 10},
            'hinging':   {'avg_hip': 130, 'tolerance': 15},
            'bottom':    {'avg_hip': 95,  'tolerance': 15},
            'returning': {'avg_hip': 130, 'tolerance': 15},
        }
    ...
    def update_rep_counter(self, angles, feedback, voice):
        ...
        if self.phase == "standing" and avg_hip < 155:
            ...
            self.phase = "bottom"
        elif self.phase == "bottom" and avg_hip > 120:
