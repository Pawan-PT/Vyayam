"""
Banded RDL V2 - Romanian deadlift with resistance band under feet
...
class BandedRdlV2:
    """Banded RDL - RDL with resistance band under feet for hip extension loading.
    ...
    Biomechanics:
    - Band under feet held at hip level provides ascending resistance
    - Same mechanics as bodyweight RDL: neutral spine, hip-back hinge
    - Band increases load at hip extension (top of movement)
    - Eccentric control more challenging with band tension
    ...
    def get_target_poses(self):
        return {
            ...
            'hinging':   {'avg_hip': 130, 'avg_knee': 162, 'tolerance': 15},
            'bottom':    {'avg_hip': 90,  'avg_knee': 158, 'tolerance': 15},
            'returning': {'avg_hip': 130, 'avg_knee': 162, 'tolerance': 15},
        }
    ...
    def validate_form(self, angles, phase):
        ...
        if phase in ('hinging', 'bottom'):
            if knee < 140:
                feedback['knee'] = JointFeedback(
                    joint='knee', status=FormStatus.WARNING,
                    message="Keep slight knee bend — not a squat"
                )
            if hip > 115 and phase == 'bottom':
                feedback['depth'] = JointFeedback(
                    joint='hip', status=FormStatus.WARNING,
                    message="Hinge further — push hips back against band tension"
                )
        if phase == 'returning':
            # Band adds tension on the way up — remind patient to drive hips through
            pass
        return feedback
    ...
    def update_rep_counter(self, angles, feedback, voice):
        rep_done = False
        warnings = []
        angles_dict = angles if isinstance(angles, dict) else {'avg_hip': 175}
        avg_hip = angles_dict.get('avg_hip', 175)

        if self.phase == "standing" and avg_hip < 155:
            self.phase = "hinging"
            self.tempo_detector.start_phase('hinging')
        elif self.phase == "hinging" and avg_hip < 105:
            self.phase = "bottom"
        elif self.phase == "bottom" and avg_hip > 115:
            self.phase = "returning"
            self.tempo_detector.start_phase('returning')
        elif self.phase == "returning" and avg_hip >= 168:
            rep_done = True
            self.phase = "standing"
