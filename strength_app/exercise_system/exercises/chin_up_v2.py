"""
Chin-Up - Supinated grip pull-up with greater bicep involvement
...
class ChinUpV2:
    """Chin-Up - Level: intermediate | Pull pattern (bar track, supinated)

    Biomechanics:
    - Palms face body (supinated); activates biceps brachii more than pull-up
    - Same ROM requirement: dead hang → chin over bar
    - Elbow fully flexed at top (~55° avg_elbow) — squeeze at peak
    - Often easier first full vertical pull for beginners
    ...
    def calculate_angles(self, analyzer, results, shape):
        """Elbow angle + chin clearance for chin-up (supinated grip)."""
    ...
    def get_target_poses(self):
        return {
            ...
            'top':   {'avg_elbow': 55,  'chin_clear': 1, 'tolerance': 15},
    ...
    def validate_form(self, angles, phase):
        ...
        if phase == 'hang' and ea < 165:
            feedback['hang'] = JointFeedback('elbow', FormStatus.WARNING,
                'Full dead hang — supinated grip, arms straight')
        if phase == 'top' and cc == 0:
            feedback['chin'] = JointFeedback('shoulder', FormStatus.WARNING,
                'Chin over bar — squeeze biceps at top')
        return feedback
    ...
    def update_rep_counter(self, angles, feedback, voice):
        ...
        elif self.phase == 'pull' and avg_elbow < 65 and chin_clear:
            self.phase = 'top'
        elif self.phase == 'top' and avg_elbow > 75:
            self.phase = 'lower'
