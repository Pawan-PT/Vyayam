"""
SingleArmPlank V2 - Standard plank with one hand removed — anti-rotation challenge
...
class SingleArmPlankV2:
    """SingleArmPlank — Standard plank with one hand removed — anti-rotation challenge

    Level: L4 elite
    Category: core
    Movement Pattern: anti-rotation
    Target: Obliques, serratus anterior, rotator cuff, TVA

    Biomechanics:
    - Remove one hand; body must resist rotation toward the unsupported side
    - Zero hip rotation is the goal — pelvis stays square to floor
    - Supported arm directly under shoulder; unsupported arm along body or behind back
    ...
    def validate_form(self, angles, phase):
        ...
        if hip_rot > 0.35:
            feedback['hip_rotation'] = {'status': 'error', 'message': 'Resist rotation — keep hips square'}
        body = angles.get('body_line', 180)
        if body < 158:
            feedback['body_line'] = {'status': 'error', 'message': 'Maintain straight body line — no sag'}
        shoulder = angles.get('shoulder_level', 0)
        if shoulder > 20:
            feedback['shoulder'] = {'status': 'warning', 'message': 'Keep shoulders level — anti-rotate'}
        return feedback
