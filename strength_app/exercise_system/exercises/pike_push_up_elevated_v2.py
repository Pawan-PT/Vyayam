"""
Elevated Pike Push-Up - Pike push-up with feet on box, steeper shoulder angle
...
class PikePushUpElevatedV2:
    """Elevated Pike Push-Up - Level: advanced | Push pattern (handstand progression)

    Biomechanics:
    - Feet on box/chair; much steeper pressing angle approaching handstand
    - Shoulder angle target ~90–100° — near vertical press vector
    - Close handstand push-up simulation without full inversion
    - Hip angle should be <90° (very tight pike)
    ...
    def calculate_angles(self, analyzer, results, shape):
        """Same as pike but steeper shoulder angle due to foot elevation."""
        ...
        shoulder_angle = analyzer.calculate_angle(lh, ls, le)

        return {
            'left_elbow': left_elbow, 'right_elbow': right_elbow,
            'avg_elbow': avg_elbow, 'hip_angle': hip_angle,
    ...
    def get_target_poses(self):
        return {
            'start':  {'avg_elbow': 170, 'shoulder_angle': 95, 'tolerance': 10},
            'down':   {'avg_elbow': 90,  'shoulder_angle': 100, 'tolerance': 15},
            # DA-EX-phases: state machine reaches 'bottom'; it was missing
            # from the scored targets (KeyError on every rep at depth).
            'bottom': {'avg_elbow': 90,  'shoulder_angle': 100, 'tolerance': 15},
            'up':     {'avg_elbow': 170, 'shoulder_angle': 95, 'tolerance': 10},
        }

    def validate_form(self, angles, phase):
        feedback = {}
        ha = angles.get('hip_angle', 90)
        sa = angles.get('shoulder_angle', 90)
        if ha > 100:
            feedback['pike'] = JointFeedback('hip', FormStatus.WARNING,
                'Keep hips high — maintain tight pike position')
        if sa < 80:
            feedback['elevation'] = JointFeedback('shoulder', FormStatus.WARNING,
    ...
    def update_rep_counter(self, angle, feedback, voice):
        ...
        if self.phase == 'start' and avg_elbow < 150:
            ...
            self.phase = 'bottom'
        elif self.phase == 'bottom' and avg_elbow > 115:
            self.phase = 'up'
