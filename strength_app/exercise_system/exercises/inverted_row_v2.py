"""
Inverted Row - Horizontal body pull under a low bar or TRX
...
class InvertedRowV2:
    ...
    Target: latissimus dorsi, rhomboids, biceps brachii, core

    Body remains in straight plank alignment throughout the row.
    """
    ...
    def get_target_poses(self):
        return {
            ...
            'top':     {'avg_elbow': 50,  'tolerance': 15},
            'lowering':{'avg_elbow': 100, 'tolerance': 20},
        }

    def validate_form(self, angles, phase):
        feedback = {}
        body = angles.get('body_align', 175) if 'body_align' in angles else 175
        if body < 155:
            feedback['sag'] = JointFeedback(
                joint='hip', status=FormStatus.WARNING,
                message="Keep body straight — don't sag at the hips"
            )
        return feedback
    ...
    def update_rep_counter(self, angle, feedback, voice):
        ...
        elif self.phase == "pulling" and avg_elbow < 65:
            self.phase = "top"
        elif self.phase == "top" and avg_elbow > 70:
            self.phase = "lowering"
