"""
SingleLegEyesClosed V2 - Eyes-closed single-leg balance — proprioceptive and vestibular challenge
...
class SingleLegEyesClosedV2:
    """SingleLegEyesClosed — Eyes-closed single-leg balance — proprioceptive and vestibular challenge

    Level: L3 advanced
    Category: balance
    Movement Pattern: single-leg-stability
    Target: Ankle stabilizers, tibialis anterior, peroneus longus, hip abductors

    Biomechanics:
    - Remove visual input to challenge proprioceptive and vestibular systems
    - Camera tracking is minimal — exercise is primarily timer-based
    - Any foot touch-down resets the hold timer
    ...
    def calculate_angles(self, analyzer, results, shape):
        ...
        trunk_sway = abs(ls[0] - la[0]) if ls and la else 0
        return {
            'left_knee': left_knee,
    ...
    def validate_form(self, angles, phase):
        ...
        if hip_l > 30:
            feedback['hip_drop'] = {'status': 'error', 'message': 'Hip dropping — stand taller on stance leg'}
        trunk = angles.get('trunk_sway', 0)
        if trunk > 40:
            feedback['sway'] = {'status': 'warning', 'message': 'Trunk swaying — find your centre'}
        return feedback
