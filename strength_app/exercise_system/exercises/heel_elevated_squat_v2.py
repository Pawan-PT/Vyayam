"""
Heel Elevated Squat V2 - Squat with heels raised to reduce ankle mobility demand
...
class HeelElevatedSquatV2:
    """Heel Elevated Squat - Heels on small platform for increased quad activation and depth.

    Level: intermediate
    Category: strength
    Movement Pattern: squat
    Target: quadriceps (emphasis), glutes

    Biomechanics:
    - Heel elevation (~1–2 inches) shifts weight forward, enabling upright torso
    - Ankle dorsiflexion limitation bypassed — allows deeper squat
    - More upright torso than flat-foot squat — greater quad loading
    - Deeper depth expected (knee angle <85°)
        ...
        if phase == 'bottom':
            if knee > 95:
                feedback['depth'] = JointFeedback(
                    ...
                    message="Go deeper — heel elevation allows extra depth"
                )
            if torso < 70:
                feedback['torso'] = JointFeedback(
                    joint='torso', status=FormStatus.WARNING,
                    message="Stay upright — heel elevation should help"
                )
        return feedback
    ...
    def update_rep_counter(self, angle, feedback, voice):
        ...
        elif self.phase == "descending" and avg_knee < 95:
            self.phase = "bottom"
        elif self.phase == "bottom" and avg_knee > 100:
