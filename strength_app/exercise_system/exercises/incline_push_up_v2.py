"""
Incline Push-Up - Hands on bench/table, reduced load push-up in plank
...
class InclinePushUpV2:
    """Incline Push-Up - Level: foundation | Push pattern

    Biomechanics:
    - Hands on elevated surface (table/bench), body in plank
    - Elbow angle tracks full ROM — ~90° at bottom
    - Hip alignment must stay neutral — no sagging or piking
    - Controlled descent equally important as the push
    ...
    def calculate_angles(self, analyzer, results, shape):
        """Elbow angle + hip position to detect plank breakdown."""
    ...
    def validate_form(self, angles, phase):
        ...
        if ha < 160:
            feedback['hip_sag'] = JointFeedback('hip', FormStatus.WARNING,
                'Hips sagging — brace your core')
        if phase == 'down' and ea < 75:
            feedback['depth'] = JointFeedback('elbow', FormStatus.GOOD,
                'Full depth — chest near surface')
        return feedback
