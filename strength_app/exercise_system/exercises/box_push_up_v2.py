"""
Box Push-Up - Hands on low box/step, further reduced load
...
class BoxPushUpV2:
    """Box Push-Up - Level: foundation | Push pattern

    Biomechanics:
    - Hands on low box/step (~20–30 cm elevation)
    - Full depth required — chest near box surface
    - Hip neutrality essential — no sagging
    - Bridges incline push-up to floor-level push-up
    ...
    def validate_form(self, angles, phase):
        ...
        if ha < 160:
            feedback['hip_sag'] = JointFeedback('hip', FormStatus.WARNING,
                'No hip sag — keep body in a straight line')
        if phase == 'bottom' and ea > 110:
            feedback['depth'] = JointFeedback('elbow', FormStatus.WARNING,
                'Go deeper — chest toward box')
        return feedback
