"""
Archer Pull-Up - One arm does the pull-up, other arm extends on bar
...
class ArcherPullUpV2:
    """Archer Pull-Up - Level: advanced | Pull pattern (bar track, unilateral)

    Biomechanics:
    - Wide grip; one arm drives the pull-up, other extends along bar
    - Working arm: full ROM dead hang → chin clearance
    - Assist arm stays near-straight (150°+) to minimise contribution
    - Intermediate progression toward one-arm pull-up
    ...
    def calculate_angles(self, analyzer, results, shape):
        """Track both elbows; working arm = more bent."""
        ...
        working_arm = min(left_elbow, right_elbow)
        assist_arm  = max(left_elbow, right_elbow)

        return {
            'left_elbow': left_elbow, 'right_elbow': right_elbow,
            'working_arm': working_arm, 'assist_arm': assist_arm,
    ...
    def validate_form(self, angles, phase):
        feedback = {}
        aa = angles.get('assist_arm', 180)
        wa = angles.get('working_arm', 180)
        if aa < 140:
            feedback['assist'] = JointFeedback('elbow', FormStatus.WARNING,
                'Keep assist arm extended — working arm does the pull')
        if phase == 'pull' and wa > 100:
            feedback['depth'] = JointFeedback('elbow', FormStatus.WARNING,
                'Pull higher on working arm')
        return feedback
    ...
    def update_rep_counter(self, angle, feedback, voice):
        ...
        if self.phase == 'hang' and working < 140:
            self.phase = 'pull'
            self.tempo_detector.start_phase('pull')
        elif self.phase == 'pull' and working < 65:
            self.phase = 'top'
        elif self.phase == 'top' and working > 80:
            self.phase = 'lower'
            self.tempo_detector.start_phase('lower')
        elif self.phase == 'lower' and working >= 165:
            rep_done = True
            self.phase = 'hang'
