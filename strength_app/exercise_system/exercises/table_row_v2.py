"""
Table Row - Lie under table, pull chest to table edge (Australian pull-up)
...
class TableRowV2:
    """Table Row - Level: building | Pull pattern (no equipment)

    Biomechanics:
    - Lie supine below table, overhand grip on table edge, pull chest up
    - Body in plank throughout — hips don't sag or pike
    - Elbow angle ~90° at top; chest contacts table
    - 45–70% bodyweight load depending on body angle
    ...
    def calculate_angles(self, analyzer, results, shape):
        """Elbow angle + body alignment (plank below table)."""
    ...
    def validate_form(self, angles, phase):
        ...
        if ba < 165:
            feedback['plank'] = JointFeedback('hip', FormStatus.WARNING,
                'Maintain plank — hips level, core tight')
        if phase == 'top':
            feedback['chest'] = JointFeedback('shoulder', FormStatus.GOOD,
                'Chest to table — scapulae squeezed')
        return feedback
