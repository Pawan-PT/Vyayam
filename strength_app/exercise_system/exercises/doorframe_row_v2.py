"""
Doorframe Row - Stand in doorway, lean back, pull body toward door frame
...
class DoorframeRowV2:
    """Doorframe Row - Level: foundation | Pull pattern (no equipment)

    Biomechanics:
    - Stand in doorway holding both sides; lean back and row
    - Body straight from ankles to head — no hip break
    - Pull with back muscles; elbows drive back past torso
    - Lean angle determines loading — more lean = harder
    ...
    def calculate_angles(self, analyzer, results, shape):
        ...
        body_lean   = analyzer.calculate_angle(la, lh, ls)  # ankle→hip→shoulder

        return {
            'left_elbow': left_elbow, 'right_elbow': right_elbow,
            'avg_elbow': avg_elbow, 'body_lean': body_lean,
    ...
    def get_target_poses(self):
        return {
            'start': {'avg_elbow': 170, 'body_lean': 130, 'tolerance': 15},
            'pull':  {'avg_elbow': 90,  'body_lean': 130, 'tolerance': 15},
            # DA-EX-phases: 'top' and 'lower' were reachable by the state
            # machine but missing from targets (KeyError mid-rep).
            'top': {'avg_elbow': 90,  'body_lean': 130, 'tolerance': 15},
            'lower': {'avg_elbow': (90, 170), 'tolerance': 15},
        }

    def validate_form(self, angles, phase):
        feedback = {}
        bl = angles.get('body_lean', 180)
        ea = angles.get('avg_elbow', 180)
        if bl > 155:
            feedback['lean'] = JointFeedback('hip', FormStatus.WARNING,
                'Lean further back — increase difficulty')
        if phase == 'pull' and ea > 110:
            feedback['pull'] = JointFeedback('elbow', FormStatus.WARNING,
                'Pull chest all the way to frame')
        return feedback
