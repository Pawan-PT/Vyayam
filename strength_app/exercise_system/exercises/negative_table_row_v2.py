"""
Negative Table Row - Jump to top of table row, lower slowly over 5 seconds
...
class NegativeTableRowV2:
    """Negative Table Row - Level: building | Pull pattern (eccentric focus)

    Biomechanics:
    - Jump/assist to top position (chest to table), control the descent
    - Target: 5-second eccentric lowering phase
    - Body straight throughout — same plank demand as table row
    - Eccentric overload builds pulling strength faster than concentric only

    Key Landmarks (MediaPipe):
    - LEFT_SHOULDER (11), RIGHT_SHOULDER (12)
    - LEFT_ELBOW (13), RIGHT_ELBOW (14)
    - LEFT_WRIST (15), RIGHT_WRIST (16)
    - LEFT_HIP (23), LEFT_ANKLE (27)
    """
    ...
    def calculate_angles(self, analyzer, results, shape):
        ...
        body_align  = analyzer.calculate_angle(la, lh, ls)

        return {
            'left_elbow': left_elbow, 'right_elbow': right_elbow,
            'avg_elbow': avg_elbow, 'body_align': body_align,
            'joints_coords': {'ls': ls, 'le': le, 'lw': lw,
                              'rs': rs, 're': re, 'rw': rw, 'lh': lh, 'la': la},
        }

    def get_target_poses(self):
        return {
            'start':   {'avg_elbow': 90,  'body_align': 175, 'tolerance': 15},
            'descend': {'avg_elbow': 135, 'body_align': 175, 'tolerance': 15},
            'bottom':  {'avg_elbow': 170, 'body_align': 175, 'tolerance': 10},
        }

    def validate_form(self, angles, phase):
        feedback = {}
        ba = angles.get('body_align', 180)
        if ba < 165:
            feedback['plank'] = JointFeedback('hip', FormStatus.WARNING,
                'Body straight — no hip sag during descent')
        if phase == 'descend':
            elapsed = getattr(self, '_descent_time', 0)
            if elapsed < 3.0:
                feedback['tempo'] = JointFeedback('elbow', FormStatus.WARNING,
    ...
    def update_rep_counter(self, angle, feedback, voice):
        ...
        if self.phase == 'start' and avg_elbow > 110:
            self.phase = 'descend'
            self._descent_start = time.time()
            self.tempo_detector.start_phase('descend')
        elif self.phase == 'descend' and avg_elbow >= 160:
            self._descent_time = time.time() - getattr(self, '_descent_start', time.time())
            self.phase = 'bottom'
