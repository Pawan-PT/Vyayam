"""
BearCrawlWithReach V2 - Bear crawl with forward arm reach — anti-rotation challenge
...
class BearCrawlWithReachV2:
    """BearCrawlWithReach — Bear crawl with forward arm reach — anti-rotation challenge

    Level: L3 advanced
    Category: carry
    Movement Pattern: anti-rotation
    Target: Serratus anterior, obliques, hip stabilizers, coordination

    Biomechanics:
    - From bear crawl hover, reach one arm fully forward while staying still
    - Zero hip rotation during the reach — core braces anti-rotationally
    - Return hand before alternating sides
    ...
    def calculate_angles(self, analyzer, results, shape):
        ...
        left_reach = analyzer.calculate_angle(lh, ls, lw)
        hip_rotation = abs(lh[0] - rh[0]) / (abs(lh[1] - rh[1]) + 1) if lh and rh else 0
    ...
    def get_target_poses(self):
        ...
        return {
            'hover': {'spine_angle': 175, 'tolerance': 15},
            'reaching': {'left_reach': 170, 'tolerance': 15},
            'return': {'left_reach': 90, 'tolerance': 20},
        }

    def validate_form(self, angles, phase):
        """Check form against exercise-specific standards."""
        feedback = {}
        hip_rot = angles.get('hip_rotation', 0)
        if hip_rot > 0.35:
            feedback['hip_rotation'] = {'status': 'error', 'message': 'No hip rotation during reach — brace harder'}
        spine = angles.get('spine_angle', 175)
        if spine < 155:
            feedback['spine'] = {'status': 'error', 'message': 'Maintain flat spine during reach'}
        return feedback
    ...
    def update_rep_counter(self, angles, feedback, voice):
        ...
        rep_done = False
        warnings = []
        reach = angles.get('left_reach', 90)

        if self.phase == "hover" and reach > 140:
            self.phase = "reaching"
            self.tempo_detector.start_phase('reaching')
        elif self.phase == "reaching" and reach > 165:
            self.phase = "return"
        elif self.phase == "return" and reach < 100:
            rep_done = True
            self.phase = "hover"
