"""
Prone Y-T-W - Face-down scapular retraction drills in three arm positions
...
class ProneYTWV2:
    """Prone Y-T-W - Level: foundation | Pull pattern (scapular health)

    Biomechanics:
    - Y: arms ~135° from body, thumbs up — lower trapezius
    - T: arms straight out ~90°, palms down — mid trapezius + rhomboids
    - W: elbows bent ~90°, external rotation — posterior rotator cuff
    - All performed face down; slow, controlled 2-second holds
    ...
    def calculate_angles(self, analyzer, results, shape):
        """Arm elevation angle for each Y/T/W shape."""
        ...
        left_arm_elevation  = analyzer.calculate_angle(lh, ls, le)
        right_arm_elevation = analyzer.calculate_angle(lh, rs, re)
        avg_elevation       = (left_arm_elevation + right_arm_elevation) / 2
        left_elbow          = analyzer.smooth_angle(analyzer.calculate_angle(ls, le, lw), 'left_elbow')

        return {
            'left_arm_elevation': left_arm_elevation, 'right_arm_elevation': right_arm_elevation,
            'avg_elevation': avg_elevation, 'left_elbow': left_elbow,
    ...
    def validate_form(self, angles, phase):
        feedback = {}
        ae = angles.get('avg_elevation', 0)
        if phase == 'hold' and ae < 70:
            feedback['lift'] = JointFeedback('shoulder', FormStatus.WARNING,
                'Lift arms higher — squeeze shoulder blades')
        if phase == 'hold' and ae >= 80:
            feedback['hold'] = JointFeedback('shoulder', FormStatus.GOOD,
    ...
    def update_rep_counter(self, angle, feedback, voice):
        ...
        if self.phase == 'start' and ae > 60:
            self.phase = 'lift'
            self.tempo_detector.start_phase('lift')
        elif self.phase == 'lift' and ae > 80:
            self.phase = 'hold'
        elif self.phase == 'hold' and ae < 50:
            self.phase = 'lower'
            self.tempo_detector.start_phase('lower')
        elif self.phase == 'lower' and ae < 25:
            rep_done = True
            self.phase = 'start'
