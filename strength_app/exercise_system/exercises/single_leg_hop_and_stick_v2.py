"""
SingleLegHopAndStick V2 - Single-leg hop with 3-second stick landing — deceleration control
...
class SingleLegHopAndStickV2:
    """SingleLegHopAndStick — Single-leg hop with 3-second stick landing — deceleration control

    Level: L4 elite
    Category: balance
    Movement Pattern: single-leg-landing
    Target: Quads, glutes, ankle stabilizers, hip abductors

    Biomechanics:
    - Hop forward off one foot, land on same foot, hold for 3 seconds
    - Landing mechanics: soft knee, hip abducted, no trunk wobble
    - The 3-second hold proves neuromuscular control was achieved
    ...
    def calculate_angles(self, analyzer, results, shape):
        ...
        hip_abduction = analyzer.calculate_angle(ls, lh, lk)
        return {
            'left_knee': left_knee,
    ...
    def get_target_poses(self):
        ...
        return {
            ...
            'airborne': {'hip_height_ratio': 0.35, 'tolerance': 0.1},
            'landing': {'left_knee_land': 110, 'tolerance': 20},
            'sticking': {'left_knee_land': 100, 'tolerance': 15},
        }
    ...
    def validate_form(self, angles, phase):
        ...
        if valgus > 0.4:
            feedback['valgus'] = {'status': 'error', 'message': 'Knee caving — push knee out over toes'}
        knee = angles.get('left_knee_land', 170)
        if knee > 155 and phase == 'sticking':
            feedback['stick'] = {'status': 'warning', 'message': 'Soften the landing — bend into the stick'}
        return feedback
    ...
    def update_rep_counter(self, angles, feedback, voice):
        ...
        self._stick_timer = getattr(self, '_stick_timer', None)

        if self.phase == "loaded" and hip_h < 0.38:
            self.phase = "airborne"
        elif self.phase == "airborne" and hip_h > 0.45:
            self.phase = "landing"
            self._stick_timer = None
        elif self.phase == "landing" and knee < 115:
            self.phase = "sticking"
            import time as _t; self._stick_timer = _t.time()
        elif self.phase == "sticking":
            import time as _t
            if self._stick_timer and (_t.time() - self._stick_timer) >= 3.0:
                rep_done = True
                self.phase = "loaded"
                self._stick_timer = None
