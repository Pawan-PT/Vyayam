"""
Dip Progression V2 - Parallel bar or chair dip
...
class DipProgressionV2:
    """Dip - Parallel bar dip or between two sturdy chairs.

    Level: building (L4)
    Category: strength
    Movement Pattern: push
    Target: triceps brachii, anterior deltoid, lower pectorals

    Biomechanics:
    - Arms support body weight; elbows bend to lower body
    - Elbow target angle: ~90° at bottom
    - Slight forward lean (10–15°) engages chest; upright targets triceps
    - Shoulder depression maintained throughout (no shrugging)
    - Full elbow extension at top without hyperextending
    ...
    def calculate_angles(self, analyzer, results, shape):
        ...
        # Shoulder elevation: if shoulder y moves up significantly during rep = shrugging
        shoulder_mid_y = (ls[1] + rs[1]) / 2
        hip_mid_y = (lh[1] + rh[1]) / 2
        shoulder_hip_ratio = shoulder_mid_y / max(1, hip_mid_y)

        return {
            'avg_elbow': avg_elbow, 'shoulder_hip_ratio': shoulder_hip_ratio,
    ...
    def validate_form(self, angles, phase):
        feedback = {}
        elbow = angles.get('avg_elbow', 170)
        if phase == 'down' and elbow > 110:
            feedback['depth'] = JointFeedback(
                joint='elbow', status=FormStatus.WARNING,
                message="Lower further — aim for 90° elbow angle"
            )
        return feedback
    ...
    def update_rep_counter(self, angle, feedback, voice):
        ...
        elif self.phase == "down" and avg_elbow >= 158:
            rep_done = True
            self.phase = "up"
