"""
Bedsheet Row - Loop bedsheet over door, sit back and row with full range
...
class BedsheetRowV2:
    """Bedsheet Row - Level: building | Pull pattern (no equipment)

    Biomechanics:
    - Bedsheet looped over top of door; hold both ends, walk feet forward
    - Greater range of motion than towel row due to attachment height
    - Full arm extension at bottom; pull to full elbow flexion at top
    - Scapular retraction and depression at peak contraction
    ...
    def calculate_angles(self, analyzer, results, shape):
        """Elbow angle + trunk lean for bedsheet row."""
        ...
        trunk_angle = analyzer.calculate_angle(ls, lh, lk)
    ...
    def get_target_poses(self):
        return {
            'start': {'avg_elbow': 170, 'trunk_angle': 45, 'tolerance': 15},
            'pull':  {'avg_elbow': 90,  'trunk_angle': 45, 'tolerance': 15},
            # DA-EX-phases: 'top' and 'lower' were reachable by the state
            # machine but missing from targets (KeyError mid-rep).
            'top': {'avg_elbow': 90,  'trunk_angle': 45, 'tolerance': 15},
            'lower': {'avg_elbow': (90, 170), 'tolerance': 15},
        }

    def validate_form(self, angles, phase):
        feedback = {}
        ta = angles.get('trunk_angle', 90)
        ea = angles.get('avg_elbow', 180)
        if ta > 70:
            feedback['lean'] = JointFeedback('hip', FormStatus.WARNING,
                'Lean further back for more range of motion')
        if phase == 'start' and ea < 155:
            feedback['extension'] = JointFeedback('elbow', FormStatus.WARNING,
                'Fully extend arms at bottom — full ROM')
        return feedback
