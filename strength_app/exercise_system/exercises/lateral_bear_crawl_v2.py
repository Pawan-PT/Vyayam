"""
LateralBearCrawl V2 - Bear crawl moving sideways — rotational stability and coordination
...
class LateralBearCrawlV2:
    """LateralBearCrawl — Bear crawl moving sideways — rotational stability and coordination

    Level: L2 intermediate
    Category: carry
    Movement Pattern: lateral-locomotion
    Target: Hip abductors, shoulders, obliques, coordination

    Biomechanics:
    - Same bear crawl position; step sideways (ipsilateral hand-foot move together)
    - Maintain hover and spine position while changing direction
    - Equal distance in each direction — track each side separately
    ...
    def calculate_angles(self, analyzer, results, shape):
        ...
        lateral_drift = abs(ls[0] - lh[0]) if ls and lh else 0
        return {
            'left_knee': left_knee,
    ...
    def validate_form(self, angles, phase):
        ...
        if spine < 155:
            feedback['spine'] = {'status': 'error', 'message': 'Keep spine neutral during lateral step'}
        hip_l = angles.get('hip_level', 0)
        if hip_l > 20:
            feedback['hip_level'] = {'status': 'warning', 'message': 'Keep hips level during lateral movement'}
        return feedback
