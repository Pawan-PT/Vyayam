"""
BandWoodchop V2 - Diagonal band chop for rotational power and core control
...
class BandWoodchopV2:
    """BandWoodchop — Diagonal band chop for rotational power and core control

    Level: L3 advanced
    Category: core
    Movement Pattern: rotation
    Target: Obliques, lats, hip external rotators, gluteus maximus

    Biomechanics:
    - Movement initiates from hips, not arms — drive the rotation from the pelvis
    - Full arc: from high outside shoulder down to opposite low hip
    - Maintain athletic stance — slight squat, weight through heels
    ...
    def calculate_angles(self, analyzer, results, shape):
        ...
        trunk_rotation = analyzer.calculate_angle(ls, lh, rh)
        arm_path_high = analyzer.calculate_angle(lw, ls, lh)
        arm_path_low = analyzer.calculate_angle(rw, rh, lh)
        ...
        return {
            ...
            'arm_path_high': arm_path_high,
            'arm_path_low': arm_path_low,
            'hip_flex': hip_flex,
            'joints_coords': {
                'lh': lh, 'lk': lk, 'la': la,
                'rh': rh, 'rk': rk, 'ra': ra,
    ...
    def get_target_poses(self):
        ...
        return {
            'high': {'arm_path_high': 160, 'tolerance': 20},
            'chopping': {'trunk_rotation': 140, 'tolerance': 20},
            'low': {'arm_path_low': 50, 'tolerance': 20},
            'returning': {'trunk_rotation': 160, 'tolerance': 20},
        }

    def validate_form(self, angles, phase):
        """Check form against exercise-specific standards."""
        feedback = {}
        trunk = angles.get('trunk_rotation', 160)
        if trunk < 110:
            feedback['trunk'] = {'status': 'warning', 'message': 'Full rotation through thoracic spine'}
        hip = angles.get('hip_flex', 180)
        if hip > 175:
            feedback['hip'] = {'status': 'warning', 'message': 'Keep slight squat — hips drive rotation'}
        return feedback
    ...
    def update_rep_counter(self, angles, feedback, voice):
        ...
        rep_done = False
        warnings = []
        arm_high = angles.get('arm_path_high', 160)
        arm_low = angles.get('arm_path_low', 160)
        trunk = angles.get('trunk_rotation', 160)

        if self.phase == "high" and arm_high < 140:
            self.phase = "chopping"
            self.tempo_detector.start_phase('chopping')
        elif self.phase == "chopping" and arm_low < 70:
            self.phase = "low"
        elif self.phase == "low" and arm_low > 100:
            self.phase = "returning"
            self.tempo_detector.start_phase('returning')
        elif self.phase == "returning" and arm_high > 150:
            rep_done = True
            self.phase = "high"
