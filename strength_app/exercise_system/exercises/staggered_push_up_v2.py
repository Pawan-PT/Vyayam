"""
Staggered Push Up V2 - One hand forward, one back for rotational stability
...
class StaggeredPushUpV2:
    """Staggered Push-Up - One hand forward, one at chest; switch each set.

    Level: intermediate (L3)
    Category: strength
    Movement Pattern: push
    Target: pectorals, triceps, anti-rotation core

    Biomechanics:
    - One hand placed ~6 inches forward of normal position, one ~6 inches back
    - Asymmetric loading challenges rotational stability of the trunk
    - Shoulders should stay level (no twisting) throughout
    - Alternate hand positions between sets
    ...
    def calculate_angles(self, analyzer, results, shape):
        ...
        # Shoulder level: y-difference between left and right shoulder
        shoulder_level_diff = abs(ls[1] - rs[1])

        return {
            'left_elbow': left_elbow, 'right_elbow': right_elbow,
            'avg_elbow': avg_elbow, 'shoulder_level_diff': shoulder_level_diff,
    ...
    def validate_form(self, angles, phase):
        ...
        if phase == 'down' and shoulder_diff > 20:
            feedback['rotation'] = JointFeedback(
                joint='shoulder', status=FormStatus.WARNING,
                message="Keep shoulders level — resist the rotation"
            )
        return feedback
