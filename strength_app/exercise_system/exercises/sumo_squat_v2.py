"""
Sumo Squat V2 - Wide stance squat targeting inner thighs and glutes
...
class SumoSquatV2:
    """Sumo Squat - Wide stance squat with toes pointed out ~45°.

    Level: building
    Category: strength
    Movement Pattern: squat
    Target: adductors, glutes, quadriceps, hamstrings

    Biomechanics:
    - Feet wider than shoulder-width, toes out 45°
    - Knees track in line with toes (outward push)
    - Upright torso — no forward lean
    - Squat between legs, not over them
    ...
    def calculate_angles(self, analyzer, results, shape):
        """Calculate knee angles and hip width for stance detection."""
        ...
        # Knee tracking: compare knee x vs hip x — knees should be outside hips
        left_knee_offset = lk[0] - lh[0]   # positive = knee outside hip
        right_knee_offset = rh[0] - rk[0]  # positive = knee outside hip
        ...
        return {
            ...
            'avg_knee': avg_knee,
            'avg_torso': avg_torso,
            'left_knee_offset': left_knee_offset,
            'right_knee_offset': right_knee_offset,
            'joints_coords': {
                'lh': lh, 'lk': lk, 'la': la,
                'rh': rh, 'rk': rk, 'ra': ra,
                'ls': ls, 'rs': rs
    ...
    def validate_form(self, angles, phase):
        """Check knee tracking outward and upright torso."""
        feedback = {}
        left_offset = angles.get('left_knee_offset', 0)
        right_offset = angles.get('right_knee_offset', 0)
        torso = angles.get('avg_torso', 90)

        if phase in ('descending', 'bottom'):
            if left_offset < -10 or right_offset < -10:
                feedback['valgus'] = JointFeedback(
                    joint='knee', status=FormStatus.WARNING,
                    message="Push knees out — track over toes"
                )
            if torso < 65:
                feedback['torso'] = JointFeedback(
                    joint='torso', status=FormStatus.WARNING,
                    message="Keep chest up — stay upright"
                )
        return feedback
    ...
    def update_rep_counter(self, angle, feedback, voice):
        ...
        elif self.phase == "descending" and avg_knee < 105:
            self.phase = "bottom"
        elif self.phase == "bottom" and avg_knee > 110:
