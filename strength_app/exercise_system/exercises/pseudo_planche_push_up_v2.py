"""
Pseudo Planche Push-Up - Forward lean with rotated hands, anterior shoulder demand
...
class PseudoPlanchePushUpV2:
    """Pseudo Planche Push-Up - Level: advanced | Push pattern (planche progression)

    Biomechanics:
    - Hands rotated out (fingers sideways/back), significant forward lean
    - Shoulder joint operates in high protraction + depression — planche prep
    - Shoulder angle target ~20–30° (wrists behind shoulders plane)
    - Elbow tracking close to body at bottom
    ...
    def calculate_angles(self, analyzer, results, shape):
        """Shoulder angle (forward lean) + elbow angle."""
        ...
        shoulder_angle = analyzer.calculate_angle(lh, ls, le)  # lean angle
    ...
    def get_target_poses(self):
        return {
            'start':  {'avg_elbow': 170, 'shoulder_angle': 30, 'tolerance': 10},
            'down':   {'avg_elbow': 90,  'shoulder_angle': 20, 'tolerance': 15},
            # DA-EX-phases: state machine reaches 'bottom'; it was missing
            # from the scored targets (KeyError on every rep at depth).
            'bottom': {'avg_elbow': 90,  'shoulder_angle': 20, 'tolerance': 15},
            'up':     {'avg_elbow': 170, 'shoulder_angle': 30, 'tolerance': 10},
        }
    ...
    def validate_form(self, angles, phase):
        ...
        if sa > 50:
            feedback['lean'] = JointFeedback('shoulder', FormStatus.WARNING,
                'Lean further forward — shoulders over wrists')
        if phase == 'down':
            feedback['note'] = JointFeedback('shoulder', FormStatus.GOOD,
                f'Lean angle: {sa:.0f}° — planche prep loading')
        return feedback
