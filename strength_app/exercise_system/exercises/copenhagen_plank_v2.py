"""
CopenhagenPlank V2 - Side plank with top leg elevated — adductor and core hold
...
class CopenhagenPlankV2:
    """CopenhagenPlank — Side plank with top leg elevated — adductor and core hold

    Level: L3 advanced
    Category: core
    Movement Pattern: anti-lateral-flexion
    Target: Hip adductors, obliques, gluteus medius, hip abductors

    Biomechanics:
    - Top foot rests on elevated surface (bench/chair) — hip adduction provides support
    - Bottom leg may hang or lightly touch floor depending on variant
    - Maintain straight body line from head to hip
    ...
    def calculate_angles(self, analyzer, results, shape):
        ...
        hip_height = lh[1] / shape[0] if lh else 0.5
        top_leg_angle = analyzer.calculate_angle(lh, lk, la)
        return {
            'left_knee': left_knee,
    ...
    def validate_form(self, angles, phase):
        ...
        if body < 155:
            feedback['hip_sag'] = {'status': 'error', 'message': 'Drive hips up — maintain straight line'}
        if body > 200:
            feedback['hip_pike'] = {'status': 'warning', 'message': 'Lower hips slightly — avoid pike'}
        top_leg = angles.get('top_leg_angle', 90)
        if top_leg > 160:
            feedback['adductor'] = {'status': 'warning', 'message': 'Engage adductors — press into surface'}
        return feedback
